...
When a source file is read in, it is first converted into a  `SourceFile`:

```
pub struct SourceFile {
  src_file: String, // the name of the source file
...
describes how a single line might be reported along with the diagnostic information, but, again, a similar mechanism could be used to reporting multiple lines.
  ...
  The template is represented by the following structure:
  ...
  pub struct Diagnostic {
    ...
    stage: Stage, // lexer, parser, cp analyzer, or code gen
    level: Level, // info, warning, error
    src: String, // source code corresponding to the merged span
  }


  ```

  which is generated by the following reporting function:

  ```
    pub fn report_error(stage: Stage, level: Level, span: Span, text: String) 
 ...
 ```
  ...
  and is used by the emitter to generate a nicely-formatted error/diagnostic message.
